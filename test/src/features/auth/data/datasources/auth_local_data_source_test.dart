...
void main() {
  ...
  test(
    'Given the SharedPreferences conclude the caching of the token when save token method is called then return an unit',
    () async {
      // ARRANGE
      when(
  ...
  test(
    'Given the SharedPreferences fails to cache the token when save token method is called then throws a CacheException',
    () async {
      // ARRANGE
      when(
        () => mockSharedPreferences.setString(any(), any()),
      ...
      // ASSERT
      await expectLater(
          cacheCall(tToken), throwsA(const TypeMatcher<CacheException>()));
      verify(() => mockSharedPreferences.setString('token', tToken)).called(1);
    },
  );
}
